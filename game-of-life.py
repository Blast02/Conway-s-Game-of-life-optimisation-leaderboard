...
WHITE = (254, 254, 254)
GREEN = (0, 254, 0)
step = 10 # size of one cell
tick = 0
nb_tick = 500 # number of iterations

rng = numpy.random.default_rng(seed=42) #seed set to 42
current_grid = rng.integers(0, 2, size=(int(window_height / step), int(window_width / step)), dtype=int)
...
def draw_alive(next_grid): #draw a green rectangle to each 1 in the grid
	...
	for r in range(next_grid.shape[0]):
		for c in range(next_grid.shape[1]):
			if next_grid[r, c] == 1:
				frame.fill(GREEN, (c*step, r*step, step, step))
